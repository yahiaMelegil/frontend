...
import { getExpertResetPasswordErrors, getPasswordStrength } from '../../utils/expertAuthValidation';
import { PATHS } from '../../../../routes/paths';

export default function ExpertResetPassword() {
  const [searchParams] = useSearchParams();
  const { resetExpertPassword } = useAuth();
  ...
  const errors = useMemo(() => getExpertResetPasswordErrors({ password, confirm }), [password, confirm]);
  ...
  const onSubmit = async (event) => {
    ...
    try {
      const payload = await resetExpertPassword({
        email,
        token,
        password,
        password_confirmation: confirm,
      });
      setSuccessMessage(payload?.message || 'تم تغيير كلمة مرور الخبير بنجاح.');
      setSuccess(true);
    } catch (requestError) {
  ...
  if (success) {
    return (
      <div className="auth-form expert-auth-form expert-auth-state-card">
        <span className="expert-auth-state-icon expert-auth-state-icon--success"><AuthIcon name="check" size={29}/></span>
        <span className="expert-auth-kicker">تم التحديث</span>
        <h1>تم تغيير كلمة المرور</h1>
        <p>{successMessage}</p>
        <Link className="auth-primary-btn expert-auth-primary-btn" to={PATHS.EXPERT_LOGIN}>تسجيل الدخول <AuthIcon name="arrow" size={18}/></Link>
      </div>
    );
  }
  ...
  return (
    <form className="auth-form expert-auth-form" onSubmit={onSubmit} noValidate>
      <div className="auth-form__heading expert-auth-form__heading">
        <span className="expert-auth-kicker"><AuthIcon name="lock" size={14}/> كلمة مرور جديدة</span>
        <h1>أنشئ كلمة مرور قوية</h1>
        <p>استخدم كلمة جديدة لهذا الحساب. رابط الاستعادة مخصص لهذه العملية ومحدود الصلاحية.</p>
      </div>
      ...
      <div className="auth-form__fields">
        <AuthInput id="expert-reset-password" label="كلمة المرور الجديدة" icon="lock" type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => { setPassword(e.target.value); setGeneralError(''); setFieldErrors({}); }} placeholder="8 أحرف أو أكثر" autoComplete="new-password" error={submitted ? (errors.password || getFirstFieldError(fieldErrors, 'password')) : getFirstFieldError(fieldErrors, 'password')} endAction={<PasswordToggle visible={showPassword} onClick={() => setShowPassword((value) => !value)} label={showPassword ? 'إخفاء كلمة المرور' : 'إظهار كلمة المرور'} />} />
        {password && (
        ...
        <AuthInput id="expert-reset-confirm" label="تأكيد كلمة المرور" icon="lock" type={showConfirm ? 'text' : 'password'} value={confirm} onChange={(e) => { setConfirm(e.target.value); setGeneralError(''); setFieldErrors({}); }} placeholder="أعد كتابة كلمة المرور" autoComplete="new-password" error={submitted ? (errors.confirm || getFirstFieldError(fieldErrors, 'password_confirmation')) : getFirstFieldError(fieldErrors, 'password_confirmation')} endAction={<PasswordToggle visible={showConfirm} onClick={() => setShowConfirm((value) => !value)} label={showConfirm ? 'إخفاء تأكيد كلمة المرور' : 'إظهار تأكيد كلمة المرور'} />} />
      </div>

      <div className="expert-auth-info-row expert-auth-info-row--spaced"><AuthIcon name="shield" size={17}/><span>بعد نجاح التغيير، يتم حذف Tokens القديمة ولا يتم الاعتماد على أي جلسة خبير سابقة.</span></div>
      <button className="auth-primary-btn expert-auth-primary-btn" type="submit" disabled={isSubmitting || !hasResetParams} aria-busy={isSubmitting}>
      ...
      <p className="auth-switch">تحتاج رابطًا جديدًا؟ <Link to={PATHS.EXPERT_FORGOT_PASSWORD}>طلب استعادة جديدة</Link></p>
    </form>
  );
}
